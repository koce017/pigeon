...
namespace Kostic017.Pigeon.Errors
{
    class CodeErrorBag : IEnumerable<CodeError>
    {
        ...
        {
            Report($"The variable '{name}' does not exist in the current context", textSpan);
        }

        internal void ReportUndeclaredFunction(TextSpan textSpan, string name)
        {
            Report($"The function '{name}' does not exist in the current context", textSpan);
        }
        ...
        {
            Report($"Index {index} is out of bounds for length {length}", textSpan);
        }
